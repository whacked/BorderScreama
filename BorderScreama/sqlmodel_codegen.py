        ...
        for key, table in foreign_keys.items():
            if hasattr(table, "back_populates"):
                foreign_models.append(
                    f'    {key}: List["{table.model_name}"] = Relationship(back_populates="{table.back_populates}", link_model=%(link_model)s)'
                )
            else:
                foreign_models.append(
                    f'    {key}_id: Optional[int] = Field(default=None, foreign_key="{table.name}.id")'
                )
        ...
        join_table_name = (
            "".join(
                list(
                    [
                        table.model_name
                        for table in sorted(join_tables, key=lambda a: a.name)
                    ]
                )
            )
            + "Link"
    ...
    for class_block in output_code_blocks:
        ...
        for line in class_block.splitlines():
            if line.startswith(("from ", "import ")):
                ...
                class_lines.append(
                    line.replace("= None", "= Field(default=None, primary_key=True)")
                )
                continue
            elif line == "":
                continue
                ...
                class_lines.append(
                    line.replace("(BaseModel):", "(SQLModel, table=True):")
                )
                continue
